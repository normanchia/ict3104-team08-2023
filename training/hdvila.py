...
class HDVilaDataset(Dataset):
    ...
    def __init__(
        ...
        if dataset_set == "train":
            self.text_name = "caption_rm2048_train_10.csv"
        else:
            self.text_name = "caption_rm2048_train_10.csv"
    ...
    def _load_metadata(self):
        ...
        with open(caption_path, "r", encoding="utf-8") as csvfile:  # 41s
            ...
            for row in reader:
                if row["clip_id"] != last_clip_id:
                    count += 1
                    if (
                        count >= (total_count // self.all_rank) * self.all_rank
                    ):  # drop last
                        break
                    last_clip_id = row["clip_id"]
                    if count % self.all_rank == self.global_rank:
                        self.metadata.append(
                            [("%02d" % int(row["part_id"])) + row["clip_id"]]
                        )
        ...
        end_time = time.time()
        print(
            "load %d - %d items use time: %.1f;"
            % (len(self.metadata), count, end_time - start_time)
        )
    ...
    def _get_video_path(self, sample):
        ...
        video_path = os.path.join(
            self.data_dir, "part_%d" % part_id, "video_clips", clip_id
        )
        return video_path
